...
for filename in glob.glob(os.path.join(directory_name, '*.tstk')):
    ...
    theme_dict = putty_colors_render_template.process_theme(json.loads(x))
    ...
    reg_entries = putty_colors_render_template.render_template(theme_dict, template_filename, process_dict=False)  # TODO see comment above, add sanity check parameter
